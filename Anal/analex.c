...
//  AnalexSedelex
//
//  Created by Joao Sisanoski on 4/25/16.
...
int linha=1;
char *buffer;
char palavras_reservadas[24][MAX_ID] = {
    "ALGORITMO",
    "CARACTERE",
    "DIV",
    "E",
...
TInfoAtomo *avanca_um(char **);
  void read_file(FILE*);
char *carrega_buffer(char *path){
        FILE* arquivo_entrada  = fopen(path,"r");
...
TInfoAtomo *avanca_um(char **bufferStart)
{
    buffer = bufferStart;
    char c;
    char *init;
    TInfoAtomo *info_atomo = malloc(sizeof(TInfoAtomo *));
